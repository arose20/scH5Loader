...
def create_anndata_subset(input_settings):#, **kwargs): 
    ...
    with h5py.File(input_settings['data_dir'], 'r') as file:
        ...
        # Check if all values in filter_values_obs are present
        missing_values = set(input_settings['filter_values_obs']) - set(col_data.iloc[:, 0])
...
def create_anndata_subset_check(
    ...
    filter_values_obs : list,#, 'Haematopoeitic_lineage', 'Endothelial'],
    additional_cols_keep_obs : list, # leave empty will install all
