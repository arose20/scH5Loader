...
def create_obs_subset(
    ...
    with h5py.File(data_dir, "r") as file:
        ...
        original_index = pd.DataFrame(
            index=np.vectorize(lambda x: x.decode("utf-8"))(
                np.array(file["obs"]["_index"], dtype=object)
            )
        )
        ...
        if missing_values:
            raise ValueError(
                f"The following values in filter_values_obs are not present in filter_column: {missing_values}"
            )
        ...
        col_data.insert(
            0,
            "Original_index_position",
            original_index.loc[
                original_index.index.isin(col_data.index), "Original_index_position"
            ],
        )
        ...
        original_col_order = ["Original_index_position"] + list(
            file["obs"].attrs["column-order"]
        )
        out_df = out_df.filter(items=original_col_order)
        out_df = out_df.loc[:, ~out_df.columns.duplicated()]

    print(
        f"\033[1mDataFrame output for only columns of interest for cells selected by filtering {filter_column}:\033[0m\n"
    )
...
def grab_row_values(
    ...
    # Initalise empty lists - lists dynamic so adding then converting to numpy array can be faster than initialising entire size of the required array
    selected_rows_data = []
    ...
    for row_idx in tqdm(
        rows_to_load,
        desc=f"Processing Rows for {description}",
        unit="row",
        position=0,
        leave=True,
    ):
...
def create_anndata_subset(
    ...
    with h5py.File(data_dir, "r") as file:
        ...
        (
            ...
            rows_to_load, data_dset, indices_dset, indptr_dset, "main counts data"
        )
        ...
        if keep_obsm:
            if not filter_obsm:
                obsm = {
                    x: anndata._io.h5ad.read_elem(file["obsm"][x])[rows_to_load]
        ...
        if keep_obsp:
            if not filter_obsp:
                obsp = {
                    x: anndata._io.h5ad.read_elem(file["obsp"][x])[rows_to_load][
                        :, rows_to_load
                    ]
        ...
        if keep_varm:
            if not filter_varm:
                ...
                varm = {
                    x: anndata._io.h5ad.read_elem(file["varm"][x])
        ...
        if keep_varp:
            if not filter_varp:
                ...
                varp = {
                    x: anndata._io.h5ad.read_elem(file["varp"][x])
